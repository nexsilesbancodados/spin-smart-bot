...
const NumberTicker = memo(({ numbers }: Props) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 glass-strong border-t border-border/10 safe-bottom">
      <div className="max-w-2xl mx-auto flex items-center gap-2 px-4 py-2.5 overflow-x-auto scrollbar-none">
        <div className="flex items-center gap-2 shrink-0 mr-1 pr-3 border-r border-border/20">
          <div className="w-2 h-2 rounded-full bg-primary animate-pulse shadow-[0_0_8px_hsl(var(--primary)/0.4)]" />
        ...
        {last15.map((n, i) => (
          <motion.div
            ...
            className={`rounded-xl flex items-center justify-center font-black text-white shrink-0 border border-white/5 shadow-sm ${numBg(n)} ${
              i === 0
                ? 'w-10 h-10 text-[13px] ring-2 ring-primary/50 shadow-lg shadow-primary/15 ring-offset-1 ring-offset-background'
                : i < 3
                ? 'w-8 h-8 text-[11px]'
                : 'w-7 h-7 text-[10px]'
