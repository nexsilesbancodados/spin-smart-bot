...
const Navbar = memo(({
  ...
  return (
    <nav className="border-b border-border/30 z-50 shrink-0 glass">
      <div className="max-w-[1400px] mx-auto px-4 flex items-center justify-between h-12">
        {/* Brand */}
        <div className="flex items-center gap-2.5">
          <div className="relative">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/25 flex items-center justify-center shadow-sm shadow-primary/10">
              <CircleDot className="w-4 h-4 text-primary" />
            </div>
            {isPolling && (
              <div className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-primary animate-pulse shadow-[0_0_8px_hsl(var(--primary)/0.5)]" />
            )}
          </div>
          <div className="flex flex-col leading-none">
            ...
            <span className="text-[7px] text-muted-foreground/40 tracking-wider font-mono">v5.0</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 text-[10px] font-mono font-bold">
            ...
            <span className={`px-2.5 py-1 rounded-md border backdrop-blur-sm ${
              isWinning ? 'bg-neon-green/10 text-neon-green border-neon-green/15' : 'bg-destructive/10 text-destructive border-destructive/15'
            }`}>
              {winPct}%
            </span>
          </div>
          ...
          {autoLearnStatus !== 'idle' && aiEnabled && (
            <div className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-purple-500/8 border border-purple-500/15 animate-pulse">
        ...
        <div className="flex items-center gap-1.5">
          <button onClick={triggerLearn} disabled={isAnalyzing || !aiEnabled}
            className="p-2 rounded-lg text-primary hover:bg-primary/8 transition-all disabled:opacity-25"
            title="Forçar Aprendizado">
            <Brain className={`w-4 h-4 ${isAnalyzing ? 'animate-spin' : ''}`} />
          </button>

          <button onClick={() => { fetchNumbers(); fetchStored(); }}
            className="p-2 rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/8 transition-all"
            title="Atualizar">
          ...
          {lastUpdate && (
            <span className="text-[8px] text-muted-foreground/40 font-mono hidden lg:inline">
