...
import { ClipboardList, Flame, Snowflake, Activity } from 'lucide-react';
...
const SessionSummary = ({ allNumbers }: Props) => {
  const stats = useMemo(() => {
    ...
    return { hot, cold, corDom, reds, blacks, hotTerminal, zeroDelay, distintos, total: s50.length, recomendacao, recColor, recBg };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass rounded-2xl overflow-hidden card-hover border border-border/20">
      {/* Header */}
      <div className="relative px-4 pt-4 pb-3">
        <div className="absolute inset-0 bg-gradient-to-r from-neon-cyan/4 via-transparent to-neon-pink/3" />
        <div className="relative flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-neon-cyan/15 to-neon-pink/10 border border-neon-cyan/20 flex items-center justify-center shadow-neon-cyan">
            <ClipboardList className="w-4 h-4 text-neon-cyan" />
          </div>
          <div className="flex-1">
            <span className="font-display text-[10px] tracking-[0.15em] font-bold text-neon-cyan uppercase">Resumo da Sessão</span>
      ...
      <div className="px-4 pb-3">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          <div className="glass rounded-xl p-3 border border-border/15 text-center">
            <div className="text-xl mb-0.5">{stats.corDom}</div>
            <div className="text-[7px] text-muted-foreground/40 font-display tracking-wider">COR</div>
          ...
          <div className="glass rounded-xl p-3 border border-border/15 text-center">
            <div className="text-[16px] font-mono font-black text-neon-cyan mb-0.5">T{stats.hotTerminal?.[0]}</div>
            <div className="text-[7px] text-muted-foreground/40 font-display tracking-wider">TERMINAL HOT</div>
          ...
          <div className={`glass rounded-xl p-3 border text-center ${
            ...
            <div className="text-[7px] text-muted-foreground/40 font-display tracking-wider">TERMINAIS</div>
          ...
          <div className={`glass rounded-xl p-3 border text-center ${
            ...
            <div className="text-[7px] text-muted-foreground/40 font-display tracking-wider">ZERO AUSENTE</div>
            <div className={`text-[9px] font-bold mt-0.5 ${
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="glass rounded-xl p-3 border border-border/15">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-[8px] font-bold text-gold font-display tracking-wider">HOT (≥3×/50)</span>
            </div>
          ...
          <div className="glass rounded-xl p-3 border border-border/15">
            <div className="flex items-center gap-1.5 mb-2">
              <Snowflake className="w-3 h-3 text-neon-cyan/60" />
              <span className="text-[8px] font-bold text-neon-cyan/60 font-display tracking-wider">COLD (0×/50)</span>
            </div>
            <div className="flex gap-1.5 flex-wrap">
      ...
      {/* Recommendation */}
      {stats.recomendacao && (
        <div className={`mx-4 mb-4 rounded-xl border p-3 text-center ${stats.recBg}`}>
